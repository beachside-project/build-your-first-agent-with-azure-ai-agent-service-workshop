...
2.  **Show the 3 most recent transaction details** (または「最新の取引詳細を3件表示して」など)
...
3.  **What are the sales by region?** (または「地域別の売上は？」など)

    **sales by region** クエリに対する LLM の応答例です：
    ...
    !!! info
        ...
        1. LLM はユーザーの質問に答えるための SQL クエリを生成します。質問 **「What are the sales by region?」** に対しては、次の SQL クエリが生成されます：
...
4.  **Show sales by category in Europe** (または「ヨーロッパでのカテゴリ別売上を表示して」など)

    この場合、さらに複雑な SQL クエリがエージェントアプリによって実行されます。

5.  **Breakout sales by footwear** (または「履物の売上内訳」など)

    エージェントがどの製品が「履物 (footwear)」カテゴリに該当するかを把握し、「**内訳 (breakout)**」という用語の背後にある意図を理解していることに注目してください。

6.  **Show sales by region as a pie chart** (または「地域別売上を円グラフで表示して」など)
